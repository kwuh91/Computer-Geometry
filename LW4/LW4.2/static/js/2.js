...
function main() {
    ...
    window.addEventListener("click", (event) => {
        ...
        function isClicked(targetX, targetY, interval = 50) {
            if (Math.abs(event.clientX - targetX) <= interval && // targetX - interval <= event.clientX <= targetX + interval
                Math.abs(event.clientY - targetY) <= interval) { // targetY - interval <= event.clientY <= targetY + interval
                    return true;
            }
        } 

        let ind = 0;
        triangle.rotatedVertices.forEach(vertex => {
            if (isClicked(vertex.x, vertex.y)) {
                triangle.selectedVertex = triangle.vertices[ind];
                triangle.selectedRotatedVertex = vertex;
            }
    ...
    window.addEventListener('wheel', (event) => {
        triangle.rotateTo(event.deltaY / 102 * 3);
    });
}
